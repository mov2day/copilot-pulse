...
export interface Alert { id: string; severity: 'ambient' | 'info' | 'warning' | 'critical'; title: string; explanation: string[]; fingerprint: string; createdAt: string; }
export interface DashboardModel { quality: DataQuality; freshness: Freshness; now: string; budgets: Budget[]; ratePerHour: number; baselineRate: number; ratio: number; weather: string; projections: Projection[]; trail: UsageSample[]; session?: FocusSession; alerts: Alert[]; }
